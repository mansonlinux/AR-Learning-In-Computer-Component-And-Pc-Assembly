...
//by Hisyam - Ref - YT - Epi 7,8,13(on spawnGridSquare added _squareList for SetIndex).
...
public class WordsGrid : MonoBehaviour
{
    ...
    //function will just loop through all of the squares and set the position for them.
        ...
        if(currentGameData != null)
        {
            ...
            foreach (var squares in currentGameData.selectedBoardData.Board)
            {
                foreach(var squareLetter in squares.Row)
                {
                    ...
                    {
                        //if cannot find a letters.
                        Debug.LogError("All field in your array should have some letters. Press Fill up with random button in your board data to add random letter. Letter: " + squareLetter);

                        //if currently in unity editor.
                        //code to stop the unity editor.
                        #if UNITY_EDITOR
                    ...
                    //if find the letters.
                    //instantiate the grid square prefab
                    else
                    {
    ...
    private Vector3 GetSquareScale(Vector3 defaultScale)
    {
        ...
        var adjustment = 0.01f; //value to scale up/down specific square based on iteration.
    ...
    //Function to check if the whole board will be fit on the screen.
    //Else its going to scale down all of the squares till displayed on screen.
    private bool ShouldScaleDown(Vector3 targetScale)
    {
        ...
        //calculation of medium of width and the height of the grid square.
        ...
        //Code to check if the grid is going outside of the screen.
